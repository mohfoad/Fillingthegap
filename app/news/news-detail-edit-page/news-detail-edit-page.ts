...
import { NewsDetailEditViewModel } from "./news-detail-edit-view-model";
...
export function onNavigatingTo(args: NavigatedData): void {
    ...
    page.bindingContext = new NewsDetailEditViewModel(page.navigationContext);
}
...
export function onDoneButtonTap(args: EventData): void {
    ...
    *************************************************************/
    ...
    const bindingContext = <NewsDetailEditViewModel>actionItem.bindingContext;

    bindingContext.saveChanges()
        .then(() => topmost().navigate({
            moduleName: "news/news-list-page",
            clearHistory: true,
            animated: true,
            transition: {
                name: "slideBottom",
...
export function onSelectorTap(args: EventData): void {
    ...
    const bindingContext = <NewsDetailEditViewModel>gridLayout.bindingContext;
    const selectedValue = bindingContext.news[tag];
    const context = { tag, selectedValue };
    const modalPagePath = "news/list-selector-modal-page/list-selector-modal-page";
    const page = <Page>gridLayout.page;

    page.showModal(modalPagePath, context, (value: string) => {
        if (value) {
            bindingContext.news[tag] = value;
        }
    }, false);
}
